...
#read data csv
def read_data():
    reader = pd.read_csv('DataTugas3.csv',delimiter=',')
    return np.array(reader)

def read_data_prediction():
    reader = pd.read_csv('TebakanTugas3.csv',delimiter=',')
    return np.array(reader)

#Save to file
def save_to_file(array_data):
    np.savetxt\
        ('TebakanTugas3.csv', array_data, fmt='%s', delimiter=',',
...
def main():
    ...
    data = read_data()
    print('--------------------------------------------------------------------------') 
    print('                   TUGAS PROGRAM 3 KECERDASAN BUATAN                      ')
