...
#read data csv
def read_data():
    reader = pd.read_csv('data/datatugas3.csv',delimiter=',')
    return np.array(reader)

def read_data_prediction():
    reader = pd.read_csv('data/data-prediction.csv',delimiter=',')
    return np.array(reader)
#Save to file
def save_to_file(array_data):
    np.savetxt\
        ('data\data-prediction.csv', array_data, fmt='%s', delimiter=',',
         header="ID, Tes Kompetensi, Kepribadian, Diterima")


#Fungsi keanggotaan Tes kompetensi
...
#Fungsi keanggotaan Kepribadian
...
def fuzzification_kompetensi(data):
    ...
    for x in range(len(data)):
        ...
        print('>','P',x+1, metriks_kompetensi)
...
def fuzzification_kepribadian(data):
    ...
    for x in range (len(data)):
        ...
        print('>','P', x+1, metriks_kepribadian)
...
def inference(kompetensi,kepribadian):
    ...
    for x in range (len(kompetensi)):
        ...
        print('>','P', x + 1, metriks_kelayakan,'I love You <3')
...
def calculate_accuracy(data, prediction):
    correct = 0
    print('Abang sayang dd <3')
    for x in range(20):
        if data[x][-1] == prediction[x][-1]:
            correct += 1
    print('> Accuracy Confirmed by :')
...
def plot_competency_graph():
    ...
    plt.plot(nilai, [kompetensi_tinggi(x) for x in nilai] , color='green', label='tinggi')
    plt.legend()
    plt.savefig('data/fungsi_keanggotaan_kompetensi.png')
    plt.show()
...
def plot_personality_graph():
    ...
    plt.plot(nilai, [kepribadian_tinggi(x) for x in nilai] , color='green', label='tinggi')
    plt.legend()
    plt.savefig('data/fungsi_keanggotaan_kepribadian.png')
    plt.show()
...
def main():
    ...
    #Mamdani Deffuzification
    print('\n=====  Defuzzification using Mamdani  =====')
    for x in range(len(kelayakan)):
        crisp = mamdani(kelayakan[x])
        print('I love you <3')
        print('> P',x+1,'=',crisp)
        # print('P',x,data[x][1],data[x][2],crisp)
        if crisp <= 65 :
            data[x][-1] = "Tidak"
        else :
            data[x][-1] = "Ya"
